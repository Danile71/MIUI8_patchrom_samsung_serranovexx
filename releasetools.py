...
def AddAssertions(info):
    ...
    for i in xrange(len(edify.script)):
        if " ||" in edify.script[i] and ("ro.product.device" in edify.script[i] or "ro.build.product" in edify.script[i]):
            edify.script[i] = edify.script[i].replace(" ||", ' || getprop("ro.product.device") == "serranoveltexx" || getprop("ro.build.product") == "serranoveltexx" || getprop("ro.product.device") == "serranove3gxx" || getprop("ro.build.product") == "serranove3gxx" || getprop("ro.build.product") == "serranove3g" ||  getprop("ro.build.product") == "serranovelte" ||')
            return
